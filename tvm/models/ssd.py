...
#
# python3 ssd.py ssd.cu ssd.schedule.json ssd.graph.json ssd.params
#####################################################
...
file_path_dir = os.path.dirname(os.path.abspath(__file__)) + '/ssd'
if not os.path.exists(file_path_dir):
    os.mkdir(file_path_dir)
device_source_file = open("ssd/"+sys.argv[1], "w")  # cu
raw_schedule_file = open("ssd/"+sys.argv[2], "w")  # json
graph_json_file = open("ssd/"+sys.argv[3], "w")  # json
param_file = open("ssd/"+sys.argv[4], "w+b")  # params
...
# mod, params = relay.testing.ssd.get_workload(
#     ssd_size=169, batch_size=batch_size, image_shape=image_shape
# )
# mod, params = relay.testing.mobilenet.get_workload(
...
tvm_input = np.ones(dshape).astype("float32")
